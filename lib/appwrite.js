...
export const getAllIncome = async(userId) => {
    try {
        const posts = await databases.listDocuments(
            databaseId,
            transactionCollectionId,
            [Query.equal('users', userId),  Query.greaterThan('amount', 0), Query.orderDesc('$createdAt')]
        )
        return posts.documents
    } catch (error) {
        throw new Error(error);
...
export const getAllExpense = async(userId) => {
    try {
        const posts = await databases.listDocuments(
            databaseId,
            transactionCollectionId,
            [Query.equal('users', userId), Query.lessThan('amount', 0), Query.orderDesc('$createdAt')]
        )
        return posts.documents
    } catch (error) {
        throw new Error(error);
...
export const getAllLatestIncome = async(userId) => {
    try {
        const posts = await databases.listDocuments(
            databaseId,
            transactionCollectionId,
            [Query.equal('users', userId),  Query.greaterThan('amount', 0), Query.orderDesc('$createdAt'), Query.limit(5)]
        )
        return posts.documents
    } catch (error) {
        throw new Error(error);
...
export const getAllLatestExpense = async(userId) => {
    try {
        const posts = await databases.listDocuments(
            databaseId,
            transactionCollectionId,
            [Query.equal('users', userId),  Query.lessThan('amount', 0), Query.orderDesc('$createdAt'), Query.limit(5)]
        )
        return posts.documents
    } catch (error) {
        throw new Error(error);
